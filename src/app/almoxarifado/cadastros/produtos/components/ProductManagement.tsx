...
export function ProductManagement() {
    ...
    const fetchProducts = async () => {
        setLoading(true);
        try {
            const productsSnapshot = await getDocs(collection(db, 'produtos_almoxarifado'));
    ...
    const handleDelete = async (productId: string) => {
        try {
            await deleteDoc(doc(db, 'produtos_almoxarifado', productId));
    ...
    const handleToggleStatus = async (productIds: string[], newStatus: 'ativo' | 'inativo') => {
        ...
        productIds.forEach(id => {
            const productRef = doc(db, 'produtos_almoxarifado', id);
    ...
    const handleBulkDelete = async () => {
        ...
        selectedProducts.forEach(id => {
            batch.delete(doc(db, 'produtos_almoxarifado', id));
        });
    ...
    const handlePrint = () => {
        generatePDF('Relatório de Produtos (Almoxarifado)', 'print');
    };

    const handleExportPDF = () => {
        generatePDF('Relatório de Produtos (Almoxarifado)', 'save');
    };
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(workbook, 'relatorio_produtos_almoxarifado.xlsx');
    };
