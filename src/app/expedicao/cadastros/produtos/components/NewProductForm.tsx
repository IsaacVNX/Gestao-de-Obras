...
const productSchema = z.object({
    ...
    metroLinear: z.coerce.number().optional(),
    pesoBruto: z.coerce.number({invalid_type_error: "O peso bruto é obrigatório."}).gt(0, "O peso bruto deve ser maior que zero."),
    
    fotos: z.array(photoSchema).optional(),
}).refine(data => {
    if (data.unidadeMedida === 'PÇ') {
        return data.metroLinear !== undefined && data.metroLinear > 0;
    }
    return true;
}, {
    message: "A metragem linear é obrigatória para unidade 'PÇ'.",
    path: ["metroLinear"],
});
...
export const NewProductForm = forwardRef<NewProductFormHandle, NewProductFormProps>(({ open, setOpen, onSaveSuccess, isClosing, handleClose }, ref) => {
    ...
    const unidadeMedida = form.watch('unidadeMedida');
    ...
    const handleCreate = async (formData: z.infer<typeof productSchema>) => {
        ...
        try {
            ...
            const q = query(collection(db, 'produtos_expedicao'), where('sku', '==', sku));
            ...
            const docRef = doc(db, 'produtos_expedicao', sku);
            await setDoc(docRef, {
                ...formData,
                status: 'ativo',
                ...
                metroLinear: formData.metroLinear || null,
        ...
        <div className={cn('h-full w-full flex flex-col', isClosing ? 'animate-slide-down' : 'animate-slide-up')}>
            <div className="w-full flex flex-col h-full bg-[#ededed]">
                ...
                <div className="p-6 flex flex-row items-center justify-between border-b bg-white shadow-md sticky top-0 z-10">
                    <div>
                        <h2 className="text-2xl font-semibold text-foreground">Adicionar Novo Produto (Expedição)</h2>
                                     ...
                                     <FormField control={form.control} name="unidadeMedida" render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <Select onValueChange={field.onChange} defaultValue={field.value} disabled={saving}>
                                                <FormControl>
                                                    <SelectTrigger className="bg-background text-black border-border"><SelectValue placeholder="Selecione" /></SelectTrigger>
                                                </FormControl>
                                                <SelectContent>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                                    <div className="md:col-span-9 grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        ...
                                        <FormField control={form.control} name="pesoLiquido" render={({ field }) => (
                                         ...
                                         {unidadeMedida === 'PÇ' && (
                                            <FormField control={form.control} name="metroLinear" render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel htmlFor="metroLinear" className="text-black">Metragem Linear</FormLabel>
                                                    <FormControl><InputWithUnit id="metroLinear" field={field} unit="m" disabled={saving}/></FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )}/>
                                        )}
