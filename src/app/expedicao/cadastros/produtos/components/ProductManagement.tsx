...
export function ProductManagement() {
    ...
    const fetchProducts = async () => {
        setLoading(true);
        try {
            const productsSnapshot = await getDocs(collection(db, 'produtos_expedicao'));
    ...
    const handleDelete = async (productId: string) => {
        try {
            await deleteDoc(doc(db, 'produtos_expedicao', productId));
    ...
    const handleToggleStatus = async (productIds: string[], newStatus: 'ativo' | 'inativo') => {
        ...
        productIds.forEach(id => {
            const productRef = doc(db, 'produtos_expedicao', id);
    ...
    const handleBulkDelete = async () => {
        ...
        selectedProducts.forEach(id => {
            batch.delete(doc(db, 'produtos_expedicao', id));
        });
    ...
    const generatePDF = (title: string, action: 'save' | 'print') => {
        ...
        if (action === 'print') {
            ...
            doc.save('relatorio_produtos_expedicao.pdf');
        }
    };

    const handlePrint = () => {
        generatePDF('Relatório de Produtos (Expedição)', 'print');
    };

    const handleExportPDF = () => {
        generatePDF('Relatório de Produtos (Expedição)', 'save');
    };
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(workbook, 'relatorio_produtos_expedicao.xlsx');
    };
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    ...
                                    <TableHead className="cursor-pointer group text-black" onClick={() => requestSort('metroLinear')}><div className="flex items-center gap-2">Metragem {getSortIcon('metroLinear')}</div></TableHead>
                                ...
                                {loading ? (
                                    ...
                                    paginatedProducts.map(product => (
                                        <TableRow key={product.id}>
                                            ...
                                            <TableCell className="text-black">{product.metroLinear ? `${product.metroLinear.toFixed(2).replace('.', ',')} m` : 'N/A'}</TableCell>
