...
export const EditClientForm = forwardRef<EditFormHandle, EditClientFormProps>(({ client, onSaveSuccess, setOpen }, ref) => {
    ...
    const handleUpdateClient = async (formData: z.infer<typeof clientSchema>) => {
        ...
        try {
            ...
            const clientDocRef = doc(db, 'clientes_expedicao', client.id);
        ...
        <div className={cn('h-full w-full flex flex-col', isClosing ? 'animate-slide-down' : 'animate-slide-up')}>
            <div className="w-full flex flex-col h-full bg-[#ededed]">
                ...
                <div className="p-6 flex flex-row items-center justify-between border-b bg-white shadow-md sticky top-0 z-10">
                    <div>
                        <h2 className="text-2xl font-semibold text-foreground">Editar Cliente</h2>
                        <p className="text-sm text-muted-foreground">
                            Modifique os detalhes do cliente abaixo.
                        </p>
                    </div>
                 ...
                 <Form {...form}>
                    <form onSubmit={(e) => { e.preventDefault(); form.handleSubmit(handleUpdateClient)(); }} className="flex flex-col flex-1 overflow-hidden">
...
EditClientForm.displayName = 'EditClientForm';
